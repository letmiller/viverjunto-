...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (!body?.ids?.length) return errorResponse('Lista de planos vazia.')

  const { results: owned } = await env.DB.prepare('SELECT id FROM goals WHERE household_id = ?')
    .bind(householdId)
    .all<{ id: string }>()
  const ownedIds = new Set(owned.map((g) => g.id))
  if (!body.ids.every((id) => ownedIds.has(id))) return errorResponse('Plano inválido.')

  await env.DB.batch(
    body.ids.map((id, i) =>
      env.DB.prepare('UPDATE goals SET sort_order = ? WHERE id = ? AND household_id = ?').bind(i, id, householdId),
    ),
  )
