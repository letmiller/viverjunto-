...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const { results: owned } = await env.DB.prepare('SELECT id FROM bills WHERE household_id = ?')
    .bind(householdId)
    .all<{ id: string }>()
  ...
  await env.DB.batch(
    body.ids.map((id, i) =>
      env.DB.prepare('UPDATE bills SET sort_order = ? WHERE id = ? AND household_id = ?').bind(i, id, householdId),
    ),
  )
